...
//   console.log("FileBox:", fileName, cids);
...
const FileBox = ({ fileName, cids }) => {
  const fetchAndDownloadFile = () => {
    ...
    // Ensure cids is an array before mapping over it
    // Convert cids object to an array of CID strings
    console.log("cids:", cids);
    const cidsArray = Object.values(cids).flat();

    Promise.all(cidsArray.map((cid) => fetchBlobFromCid(cid))).then((blobs) => {
      const commonType = blobs.length > 0 ? blobs[0].type : "application/octet-stream";
      const combinedBlob = new Blob(blobs, { type: commonType });
      const url = URL.createObjectURL(combinedBlob);
